...
instance Data.ProtoLens.Message ReadMempoolRequest where
  ...
  packedMessageDescriptor _
    = "\n\
      ...
      \\ETXtxs\CAN\SOH \ETX(\v2\RS.utxorpc.v1alpha.submit.TxInMempoolR\ETXtxs"
  packedFileDescriptor _ = packedFileDescriptor
  fieldsByTag
    = let
...
instance Data.ProtoLens.Message SubmitTxRequest where
  messageName _ = Data.Text.pack "utxorpc.v1alpha.submit.SubmitTxRequest"
  packedMessageDescriptor _
    = "\n\
...
instance Data.ProtoLens.Message SubmitTxResponse where
  messageName _ = Data.Text.pack "utxorpc.v1alpha.submit.SubmitTxResponse"
  packedMessageDescriptor _
    = "\n\
...
instance Data.ProtoLens.Message TxInMempool where
  ...
  packedMessageDescriptor _
    = "\n\
      ...
      \\ENQstage\CAN\STX \SOH(\SO2\CAN.utxorpc.v1alpha.submit.StageR\ENQstage"
  packedFileDescriptor _ = packedFileDescriptor
  fieldsByTag
    = let
        tx__field_descriptor
...
instance Data.ProtoLens.Message TxPredicate where
  ...
  packedMessageDescriptor _
    = "\n\
      ...
      \\ACKany_of\CAN\EOT \ETX(\v2\RS.utxorpc.v1alpha.submit.TxPredicateR\ENQanyOf"
  packedFileDescriptor _ = packedFileDescriptor
  fieldsByTag
    = let
...
instance Data.ProtoLens.Message WatchMempoolResponse where
  ...
  packedMessageDescriptor _
    = "\n\
      ...
      \\STXtx\CAN\SOH \SOH(\v2\RS.utxorpc.v1alpha.submit.TxInMempoolR\STXtx"
  packedFileDescriptor _ = packedFileDescriptor
  fieldsByTag
    = let
        tx__field_descriptor
